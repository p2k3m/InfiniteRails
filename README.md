...
- **Texture packs** – set `textureBaseUrl` to a bucket such as `https://your-bucket.s3.amazonaws.com/blocks` (or provide a `textures` / `textureManifest` map) to stream PNG tile maps for `grass`, `dirt`, `stone`, and `rails`. By default the game fetches each texture from `https://infinite-rails-textures.s3.amazonaws.com/blocks/<name>.png` and falls back to lightweight embedded data URLs if the bucket is unreachable.
